        ...
        with self.path.open("r", encoding="utf-8") as handle:
            for line in handle:
                ...
                for event in item["events"]:
                    ...
                    text = [
                        int(token) % 8192
                        for token in event["target_text_ids"][: self.max_text_tokens]
                    ]
                    semantic = [
                        int(token) + 1
...
def collate_nar(batch: list[dict[str, torch.Tensor]]) -> dict[str, torch.Tensor]:
    ...
    semantic_lengths = torch.tensor(
        [len(item["semantic"]) for item in batch], dtype=torch.long
    )
...
class NARSemanticGenerator(nn.Module):
    ...
    def forward(
        ...
        context = (embedded * mask.unsqueeze(-1)).sum(dim=1) / text_lengths.clamp_min(
            1
        ).unsqueeze(1)
        output_positions = torch.arange(self.max_output_tokens, device=text.device)
        hidden = context.unsqueeze(1) + self.position_embedding(
            output_positions
        ).unsqueeze(0)
        hidden = self.decoder(hidden)
        predicted_length = F.softplus(self.length_head(context).squeeze(-1)) + 1.0
        return {
            "logits": self.semantic_head(hidden),
