"""Resumable interleaved repacking for isolated long-context experiments."""
...
from training.simul_uniss.pack_sequences import iter_jsonl, make_shifted_sample, pack_samples


PACK_MARKER_SCHEMA = "simul_uniss_interleaved_repack_part_v1"
ASSEMBLY_MARKER_SCHEMA = "simul_uniss_interleaved_repack_assembly_v1"
READY_MARKER_SCHEMA = "simul_uniss_interleaved_repack_ready_v1"
...
def pack_interleaved(args: argparse.Namespace) -> dict[str, object]:
    ...
    try:
        with temporary.open("w", encoding="utf-8") as handle:
            for item in pack_samples(shifted_samples(), args.seq_length, drop_overlong=True):
        ...
        if packed_records <= 0 or represented_samples + dropped_overlong != input_records:
            raise ValueError(
                "interleaved repack accounting mismatch: "
...
def assemble_interleaved(args: argparse.Namespace) -> dict[str, object]:
    marker_path = Path(args.marker)
    if marker_path.is_file():
    ...
    for index in range(args.shard_start, args.shard_start + args.shard_count):
        ...
        part_marker = verify_part(part_dir / "PACK_INTERLEAVED_COMPLETE.json")
...
def generate_schedule(args: argparse.Namespace) -> dict[str, int]:
    ...
    stage4_iters = max(1, math.ceil(records * args.stage4_epochs / args.global_batch_size))
    stage6_iters = max(1, math.ceil(records * args.stage6_epochs / args.global_batch_size))
    stage4_warmup = max(1, math.ceil(stage4_iters * args.warmup_fraction))
    stage6_warmup = max(1, math.ceil(stage6_iters * args.warmup_fraction))
    lines = [
        "# Generated from isolated long-context interleaved repacking; do not edit by hand.",
        f'INTERLEAVED_SEQ_LENGTH="{int(marker["seq_length"])}"',
        f'INTERLEAVED_SOURCE_RECORDS="{int(marker["input_records"])}"',
        f'INTERLEAVED_REPRESENTED_SAMPLES="{int(marker["represented_samples"])}"',
        f'INTERLEAVED_DROPPED_OVERLONG="{int(marker["dropped_overlong"])}"',
        f'INTERLEAVED_PACKED_RECORDS="{records}"',
        f'STAGE4_TRAIN_ITERS="${{STAGE4_TRAIN_ITERS:-{stage4_iters}}}"',
        f'STAGE6_TRAIN_ITERS="${{STAGE6_TRAIN_ITERS:-{stage6_iters}}}"',
        f'STAGE4_QWEN_WARMUP_ITERS="${{STAGE4_QWEN_WARMUP_ITERS:-{stage4_warmup}}}"',
        f'STAGE6_QWEN_WARMUP_ITERS="${{STAGE6_QWEN_WARMUP_ITERS:-{stage6_warmup}}}"',
    ]
    atomic_write_text(Path(args.output), "\n".join(lines) + "\n")
    result = {
        "packed_records": records,
        "stage4_train_iters": stage4_iters,
        "stage6_train_iters": stage6_iters,
        "stage4_warmup_iters": stage4_warmup,
        "stage6_warmup_iters": stage6_warmup,
    }
...
def finalize(args: argparse.Namespace) -> dict[str, object]:
    ...
    marker = {
        ...
        "validation_packed_records": int(validation["packed_records"]),
    }
...
def build_parser() -> argparse.ArgumentParser:
    ...
    schedule.add_argument("--stage4-epochs", type=float, default=1.0)
    schedule.add_argument("--stage6-epochs", type=float, default=0.25)
...
def main() -> None:
    ...
    if args.command == "pack":
        pack_interleaved(args)
    elif args.command == "assemble":
        assemble_interleaved(args)
