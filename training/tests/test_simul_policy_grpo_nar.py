...
class PolicyGrpoNarTests(unittest.TestCase):
    def test_buffered_shuffle_is_deterministic_and_changes_order(self) -> None:
        ordered = list(range(20))
        first = list(buffered_shuffle(ordered, buffer_size=8, seed=17))
        repeated = list(buffered_shuffle(ordered, buffer_size=8, seed=17))
    ...
    def test_grpo_loss_is_finite(self) -> None:
        ...
        loss, metrics = grpo_loss(
            policy, reference, features, labels, group_size=4, kl_beta=0.02
        )
    ...
    def test_nar_shapes_and_loss(self) -> None:
        model = NARSemanticGenerator(
            hidden_size=32, num_layers=1, num_heads=4, max_output_tokens=12
        )
        batch = {
