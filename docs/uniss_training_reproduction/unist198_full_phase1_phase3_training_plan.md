...
后续大规模packing默认使用8个有序多进程worker。每个worker处理逻辑输入流中的连续字节范围，输出完成后按范围顺序合并，因此不会丢样本或改变样本顺序。worker边界会重置packing状态，最多增加`workers-1`条padding较多的packed记录；相对于百万级packed数据可忽略，但最终schedule仍严格按正式输出的`.count`计算。已经执行到接近完成的单进程Phase 1不重启，Phase 2和Phase 3使用并行实现。
