...
后续大规模packing默认使用16个有序多进程worker。服务器共有192个逻辑CPU和约1.9TiB内存，8 worker实测只使用约4%总CPU；16 worker在保留充分CPU、内存和磁盘写回余量的同时提高吞吐。每个worker处理逻辑输入流中的连续字节范围，输出完成后按范围顺序合并，因此不会丢样本或改变样本顺序。worker边界会重置packing状态，最多增加`workers-1`条padding较多的packed记录；相对于百万级packed数据可忽略，但最终schedule仍严格按正式输出的`.count`计算。已经完成的单进程Phase 1不重启，Phase 2和Phase 3使用并行实现。packing worker数只影响数据准备，不改变后续Phase 1至Phase 3的模型、batch、优化器、学习率或checkpoint语义。

Phase 1正式packed完成后，允许Phase 1训练与Phase 2/3 packing并行。训练runner支持`--end-phase phase1`，只读取Phase 1数据和count，不要求未来阶段packed marker。并发协调必须保证：Phase 2训练同时等待Phase 1 final checkpoint和完整packing marker；禁止启动第二个Phase 1 runner写入同一checkpoint目录。

恢复后的conda环境把cuDNN等动态库安装在`site-packages/nvidia/*/lib`。训练runner启动时自动将这些目录前置到`LD_LIBRARY_PATH`，并用`ctypes`和Transformer Engine导入做preflight，避免tmux干净环境找不到`libcudnn_graph.so.9`。
