...
RUN_NAME="uniss_phase3_event_rollout_joint_pilot15_v1"
...
TAG="${TAG:-strict_exact_runtime_v1}"
SAMPLES="${SAMPLES:-2}"
...
for value in "${CHECKPOINT}/.metadata" "${BASE_MODEL}/config.json" \
  ...
  [[ -e "${value}" ]] || { echo "Missing fixed15 evaluation input: ${value}" >&2; exit 1; }
done
...
exec "${INFERENCE_PYTHON}" -m experiments.uniss_phase3_event_rollout_joint_pilot15_v1.evaluation.evaluate_checkpoint \
