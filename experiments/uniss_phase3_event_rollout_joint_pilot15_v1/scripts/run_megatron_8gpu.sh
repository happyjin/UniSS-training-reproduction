...
for value in "${TRAJECTORY_MANIFEST}" "${VALID_TRAJECTORY_MANIFEST}" \
  ...
  [[ -f "${value}" ]] || { echo "Missing pilot15 artifact: ${value}" >&2; exit 1; }
done
...
if audit.get('status') != 'pass' and not bool(int(__import__('os').environ.get('ALLOW_SAMPLED_AUDIT','0'))):
...
if [[ -s "${TRACKER}" ]]; then
  ...
  if (( current == TRAIN_ITERS )); then echo "pilot15 run already complete at ${TRAIN_ITERS}"; exit 0; fi
