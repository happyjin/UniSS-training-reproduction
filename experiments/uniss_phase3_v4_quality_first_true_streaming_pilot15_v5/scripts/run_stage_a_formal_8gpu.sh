...
if value.get("schema_version") != "uniss_stage_a_v5_canary_gate_v1" or not value.get("passed"):
    raise SystemExit("Stage A v5 canary has not authorized formal training")
PY

RUN_ID=${RUN_ID:-stage_a_v5_adapter_formal8_$(date -u +%Y%m%dT%H%M%SZ)}
export RUN_ID
export RUN_ENTRYPOINT="${V5_ENTRYPOINT}"
export RUN_COMPILE_NAMESPACE=stage_a_v5_compile
...
export RUN_MASTER_PORT=${RUN_MASTER_PORT:-29729}
