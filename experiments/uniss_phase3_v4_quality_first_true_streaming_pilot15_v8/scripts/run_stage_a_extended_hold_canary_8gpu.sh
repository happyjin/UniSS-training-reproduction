...
RUN_ID=${RUN_ID:-stage_a_v8_hold128_canary8_$(date -u +%Y%m%dT%H%M%SZ)}
export RUN_ID
export RUN_ENTRYPOINT="${V8_ENTRYPOINT}"
export RUN_COMPILE_NAMESPACE=stage_a_v8_compile
...
export RUN_SAVE_DIR="${CHECKPOINT_ROOT}/stage_a_extended_hold_canary/${RUN_ID}"
export RUN_TENSORBOARD_DIR="${RUN_ROOT}/stage_a_extended_hold_canary/${RUN_ID}/tensorboard"
export RUN_LOG="${LOG_ROOT}/stage_a_extended_hold_canary/${RUN_ID}/train.log"
export RUN_SEQ_LENGTH=18000
export RUN_MBS=1
export RUN_GBS=128
export RUN_COVERAGE_EPOCHS=3
export RUN_PREFIX_SCHEDULE=1
export RUN_TRAIN_ITERS=255
...
export RUN_MASTER_PORT=${RUN_MASTER_PORT:-29734}
...
exec bash "${EXPERIMENT_DIR}/scripts/run_stage_a_megatron_v8.sh" "$@"
