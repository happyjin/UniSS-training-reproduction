#!/usr/bin/env python3
"""Pack ASR/MT/TTS constrained trajectories with immutable Phase3 replay."""
...
def trajectory_sample(row: dict[str, object]) -> dict[str, object]:
    ...
    return {
        ...
        "identity": f"{row['episode_id']}:g{row['group_index']}:t{row['trace_index']}:{family}",
    }
...
def replay_pack(row: dict[str, object], seq_length: int) -> dict[str, object]:
    ...
    return {
        "schema_version": "uniss_route_aligned_constrained_pack_v1",
        ...
        "replay_mask": mask,
        "family_ids": [4 if index < used else 0 for index in range(seq_length)],
        "loss_mask": mask,
...
def write_jsonl(path: Path, rows: Iterable[dict[str, object]]) -> dict[str, object]:
    ...
    family_tokens = {name: 0 for name in FAMILY_IDS}
    with path.open("wb") as handle:
        for row in rows:
            ...
            for name, family_id in FAMILY_IDS.items():
                family_tokens[name] += sum(
                    int(left == family_id and right > 0)
                    for left, right in zip(row["family_ids"], row["response_mask"])
                )
    ...
    return {
        ...
        "family_response_tokens": family_tokens,
...
def main() -> None:
    ...
    samples = [trajectory_sample(row) for row in trajectories]
    rl_packs = list(pack_samples(samples, args.seq_length))
    replay_rows = read_indexed(
        args.phase3_replay,
        range(min(len(rl_packs), len(offsets(args.phase3_replay)))),
    )
    replay_packs = [replay_pack(row, args.seq_length) for row in replay_rows]
    interleaved = []
    for index in range(max(len(rl_packs), len(replay_packs))):
        if index < len(rl_packs):
            interleaved.extend((rl_packs[index], rl_packs[index]))
        if index < len(replay_packs):
            interleaved.append(replay_packs[index])
    summary = write_jsonl(args.output, interleaved)
    summary.update(
        {
            "schema_version": "uniss_route_aligned_constrained_dataset_v1",
            "status": "passed",
