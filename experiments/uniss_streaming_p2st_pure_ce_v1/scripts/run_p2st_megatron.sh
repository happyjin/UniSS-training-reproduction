...
# This copy lives in a sibling experiment, so the environment and every path
# still come from the established experiment directory; only the entry point
# and the pool differ.
EXPERIMENT_DIR=$(cd -- "${SCRIPT_DIR}/../../uniss_phase3_v4_e2e_simuls2st_pilot15_v1" && pwd)
P2ST_DIR=$(cd -- "${SCRIPT_DIR}/.." && pwd)
...
# The established launcher never passes this, so it has always taken the CLI
# default of 0.10.  Under uniform cross-entropy it is 1.0 like every other
...
RUN_SEMANTIC_ROLLIN_END_WEIGHT=${RUN_SEMANTIC_ROLLIN_END_WEIGHT:-0.0}
RUN_SEMANTIC_ROLLIN_END_MARGIN_WEIGHT=${RUN_SEMANTIC_ROLLIN_END_MARGIN_WEIGHT:-0.0}
RUN_SEMANTIC_ROLLIN_CONTINUE_DECISION_MARGIN_WEIGHT=${RUN_SEMANTIC_ROLLIN_CONTINUE_DECISION_MARGIN_WEIGHT:-0.0}
RUN_SEMANTIC_ROLLIN_CONTINUE_DECISION_LOGIT_MARGIN=${RUN_SEMANTIC_ROLLIN_CONTINUE_DECISION_LOGIT_MARGIN:-0.0}
RUN_SEMANTIC_ROLLIN_CONTINUE_MARGIN_WEIGHT=${RUN_SEMANTIC_ROLLIN_CONTINUE_MARGIN_WEIGHT:-0.0}
RUN_SEMANTIC_ROLLIN_CONTINUE_LOGIT_MARGIN=${RUN_SEMANTIC_ROLLIN_CONTINUE_LOGIT_MARGIN:-0.0}
RUN_SEMANTIC_ROLLIN_CONTINUE_TAIL=${RUN_SEMANTIC_ROLLIN_CONTINUE_TAIL:-12}
RUN_SEMANTIC_ROLLIN_CONTINUE_RATIO=${RUN_SEMANTIC_ROLLIN_CONTINUE_RATIO:-0.5}
RUN_SEMANTIC_CONTINUE_MARGIN_WEIGHT=${RUN_SEMANTIC_CONTINUE_MARGIN_WEIGHT:-0.0}
RUN_SEMANTIC_CONTINUE_LOGIT_MARGIN=${RUN_SEMANTIC_CONTINUE_LOGIT_MARGIN:-0.0}
RUN_SEMANTIC_CONTINUE_TAIL=${RUN_SEMANTIC_CONTINUE_TAIL:-12}
RUN_SEMANTIC_BOUNDARY_BINARY_WEIGHT=${RUN_SEMANTIC_BOUNDARY_BINARY_WEIGHT:-0.0}
RUN_SEMANTIC_BOUNDARY_BINARY_LOGIT_MARGIN=${RUN_SEMANTIC_BOUNDARY_BINARY_LOGIT_MARGIN:-0.0}
...
# The prefix-to-prefix entry point reuses the established trainer's model
# provider, forward step, objective and argument surface unchanged, and swaps
# only the data path.  Every other line of this launcher is the established
# one, so a run here is comparable to B' on the same code.
ENTRYPOINT="${P2ST_DIR}/training/pretrain_p2st_megatron.py"
...
cmd=(
  ...
  --e2e-semantic-rollin-end-weight "${RUN_SEMANTIC_ROLLIN_END_WEIGHT}"
  --e2e-semantic-rollin-end-margin-weight "${RUN_SEMANTIC_ROLLIN_END_MARGIN_WEIGHT}"
  --e2e-semantic-rollin-continue-decision-margin-weight "${RUN_SEMANTIC_ROLLIN_CONTINUE_DECISION_MARGIN_WEIGHT}"
  --e2e-semantic-rollin-continue-decision-logit-margin "${RUN_SEMANTIC_ROLLIN_CONTINUE_DECISION_LOGIT_MARGIN}"
  --e2e-semantic-rollin-continue-margin-weight "${RUN_SEMANTIC_ROLLIN_CONTINUE_MARGIN_WEIGHT}"
  --e2e-semantic-rollin-continue-logit-margin "${RUN_SEMANTIC_ROLLIN_CONTINUE_LOGIT_MARGIN}"
  --e2e-semantic-rollin-continue-tail "${RUN_SEMANTIC_ROLLIN_CONTINUE_TAIL}"
  --e2e-semantic-rollin-continue-ratio "${RUN_SEMANTIC_ROLLIN_CONTINUE_RATIO}"
  --e2e-semantic-continue-margin-weight "${RUN_SEMANTIC_CONTINUE_MARGIN_WEIGHT}"
  --e2e-semantic-continue-logit-margin "${RUN_SEMANTIC_CONTINUE_LOGIT_MARGIN}"
  --e2e-semantic-continue-tail "${RUN_SEMANTIC_CONTINUE_TAIL}"
  --e2e-semantic-boundary-binary-weight "${RUN_SEMANTIC_BOUNDARY_BINARY_WEIGHT}"
  --e2e-semantic-boundary-binary-logit-margin "${RUN_SEMANTIC_BOUNDARY_BINARY_LOGIT_MARGIN}"
