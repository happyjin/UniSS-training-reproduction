...
cmd=(torchrun --nproc_per_node "${SIMUL_NPROC_PER_NODE}"
  --master_addr "${SIMUL_MASTER_ADDR}" --master_port "${STAGE8_MASTER_PORT}"
  -m training.simul_uniss.nar_semantic --schedules "${SCHEDULES_JSONL}"
  --output-dir "${STAGE8_OUTPUT_DIR}" --tensorboard-dir "${STAGE8_TENSORBOARD_DIR}"
  --device cuda --batch-size "${STAGE8_BATCH_SIZE}" --max-steps "${STAGE8_MAX_STEPS}"
