...
[[ -f "${FULL_DATA_READY_MARKER}" ]] || { echo "Full data incomplete: ${FULL_DATA_READY_MARKER}" >&2; exit 1; }
[[ -f "${SMOKE_MARKER}" ]] || { echo "Shuffle smoke incomplete: ${SMOKE_MARKER}" >&2; exit 1; }
mkdir -p "${PIPELINE_DIR}" "${LOG_DIR}"
for spec in "${stages[@]}"; do
  IFS=: read -r name root expected launcher <<< "${spec}"
  marker="${PIPELINE_DIR}/${name}.complete"
  [[ ! -f "${marker}" ]] || { echo "Skipping completed ${name}"; continue; }
  [[ ! -e "${root}" ]] || { echo "Refusing partial output: ${root}" >&2; exit 1; }
  "${launcher}"
...
printf 'completed_at=%s\nanchor=%s\ndata=%s\n' "$(date -u +%FT%TZ)" "${QWEN_CHECKPOINT_ROOT}" "${FULL_DATA_READY_MARKER}" > "${PIPELINE_MARKER}"
