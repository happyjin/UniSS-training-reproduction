...
SMOKE_NAME="shuffle_smoke_8gpu_v1"
...
[[ ! -e "${SMOKE_ROOT}" && ! -e "${SMOKE_RUN_DIR}" && ! -e "${SMOKE_LOG_DIR}" ]] || {
  echo "Refusing to overwrite existing full-data smoke output" >&2; exit 1;
}
...
printf 'completed_at=%s\ndata=%s\n' "$(date -u +%FT%TZ)" "${FULL_DATA_READY_MARKER}" > "${COMPLETE_MARKER}"
echo "Eight-GPU full-data shuffle smoke completed: ${COMPLETE_MARKER}"
