...
SAVE_DIR="${REPO_ROOT}/checkpoints/uniss_phase3_whisper_streamspeech_joint_v5/${RUN_NAME}"
TB_DIR="${REPO_ROOT}/runs/uniss_phase3_whisper_streamspeech_joint_v5/${RUN_NAME}"
LOG="${REPO_ROOT}/logs/uniss_phase3_whisper_streamspeech_joint_v5/${RUN_NAME}.log"
...
"${PYTHON}" -m torch.distributed.run \
  ...
  --joint-bridge-commitment-weight 0.25 \
  --joint-unit-upsample-ratio 48 \
  --joint-bridge-surrogate topk_soft \
  --joint-bridge-topk 8 \
  --joint-bridge-temperature 0.1 \
  --joint-max-bridge-commitment 5.0 \
  --joint-freeze-whisper-codebook \
  --joint-freeze-whisper-post-vq \
  --joint-trainable-whisper-pre-vq-layers 4 \
  --joint-lr-new-mult 1.0 \
  --joint-lr-bridge-mult 0.0 \
  --joint-lr-whisper-top-mult 0.05 \
  --joint-lr-whisper-bottom-mult 0.01 \
  --joint-lr-qwen-mult 0.01 \
  --joint-lr-qwen-io-mult 0.005 \
  "${EXTRA_REPLAY_ARGS[@]}" \
  "${EXTRA_VALID_ARGS[@]}" \
  ...
  --lr "${BASE_LR:-2e-5}" \
  --min-lr "${MIN_LR:-2e-6}" \
  --lr-warmup-iters "${LR_WARMUP_ITERS:-300}" \
