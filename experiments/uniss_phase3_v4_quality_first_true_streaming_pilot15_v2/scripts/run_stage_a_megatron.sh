...
: "${RUN_TRAIN_TEACHER_CACHE:?RUN_TRAIN_TEACHER_CACHE is required}"
...
RUN_VALID_TEACHER_CACHE=${RUN_VALID_TEACHER_CACHE:-}
...
RUN_MASTER_PORT=${RUN_MASTER_PORT:-29721}
...
required=(
  "${RUN_TRAIN_PACKS}"
  "${RUN_TRAIN_TEACHER_CACHE}"
  "$(dirname "${RUN_TRAIN_TEACHER_CACHE}")/TEACHER_CACHE_AUDIT.json"
...
for value in "${required[@]}"; do
  [[ -f "${value}" ]] || { echo "missing Stage A v2 input: ${value}" >&2; exit 1; }
done
if [[ -n "${RUN_VALID_PACKS}" ]]; then
  valid_required=(
    "${RUN_VALID_PACKS}"
    "${RUN_VALID_TEACHER_CACHE}"
    "$(dirname "${RUN_VALID_TEACHER_CACHE}")/TEACHER_CACHE_AUDIT.json"
  )
  for value in "${valid_required[@]}"; do
    [[ -f "${value}" ]] || { echo "missing Stage A v2 validation input: ${value}" >&2; exit 1; }
  done
elif [[ -n "${RUN_VALID_TEACHER_CACHE}" ]]; then
  echo "validation teacher cache was provided without validation packs" >&2
  exit 1
fi

"${PYTHON_BIN}" - "${RUN_TRAIN_PACKS}" "${RUN_TRAIN_TEACHER_CACHE}" \
  "${RUN_VALID_PACKS}" "${RUN_VALID_TEACHER_CACHE}" "${PHASE3_HF_CHECKPOINT}" <<'PY'
import json
import sys
from pathlib import Path

expected_model = Path(sys.argv[5]).resolve()
for packs_value, cache_value in ((sys.argv[1], sys.argv[2]), (sys.argv[3], sys.argv[4])):
    if not packs_value:
        continue
    packs = Path(packs_value).resolve()
    cache = Path(cache_value).resolve()
    audit = json.loads((cache.parent / "TEACHER_CACHE_AUDIT.json").read_text())
    expected = {
        "status": "complete",
    ...
    for name, value in expected.items():
        if audit.get(name) != value:
            raise SystemExit(f"teacher cache {cache} has invalid {name}: {audit.get(name)!r}")
    if Path(audit["packs"]).resolve() != packs:
        raise SystemExit(f"teacher cache {cache} belongs to a different pack file")
    if Path(audit["output"]).resolve() != cache:
        raise SystemExit(f"teacher cache {cache} audit names a different manifest")
    if Path(audit["model"]).resolve() != expected_model:
        raise SystemExit(f"teacher cache {cache} used a different Phase3 teacher")
PY
...
cmd=(
  ...
  --stage-a-train-teacher-cache "${RUN_TRAIN_TEACHER_CACHE}"
  --stage-a-teacher-lru-capacity "${RUN_TEACHER_LRU_CAPACITY}"
...
if [[ -n "${RUN_VALID_PACKS}" ]]; then
  cmd+=(
    ...
    --stage-a-valid-teacher-cache "${RUN_VALID_TEACHER_CACHE}"
