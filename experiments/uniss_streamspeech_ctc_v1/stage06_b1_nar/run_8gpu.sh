...
STAGE="$ROOT/experiments/uniss_streamspeech_ctc_v1/stage06_b1_nar"
SOURCE="$ROOT/data/processed/simul_uniss_subsecond_v1/pilot_15shard/stage_a_source/stage_a_source_manifest.jsonl"
LOG="$ROOT/logs/uniss_streamspeech_ctc_v1/stage06_b1_continuous_residual_v1.log"
...
mkdir -p "$PYTORCH_KERNEL_CACHE_PATH" "$(dirname "$LOG")"

"$PYTHON" -m torch.distributed.run --nproc_per_node=8 --master_port=29660 \
  "$STAGE/train_b1.py" \
  ...
  --output-dir "$ROOT/checkpoints/uniss_streamspeech_ctc_v1/stage06_b1_continuous_residual_v1" \
  --tensorboard-dir "$ROOT/runs/uniss_streamspeech_ctc_v1/stage06_b1_continuous_residual_v1" \
  --batch-size 16 \
  --num-workers 4 \
  --max-steps 1000 \
  --eval-every 100 \
