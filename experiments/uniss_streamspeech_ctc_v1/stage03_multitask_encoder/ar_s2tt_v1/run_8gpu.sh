...
"$PYTHON" -m torch.distributed.run --nproc_per_node=8 --master_port=29640 \
  "$STAGE/train.py" \
  ...
  --output-dir "$ROOT/checkpoints/uniss_streamspeech_ctc_v1/stage03b_ar_s2tt_v1" \
  --tensorboard-dir "$ROOT/runs/uniss_streamspeech_ctc_v1/stage03b_ar_s2tt_v1" \
  --batch-size 12 \
  --max-steps 5000 \
  --eval-every 500
