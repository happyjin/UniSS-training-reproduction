...
STAGE=$ROOT/experiments/uniss_streamspeech_ctc_v1/stage07_end_to_end_eval
SOURCE=$ROOT/data/processed/simul_uniss_subsecond_v1/pilot_15shard/stage_a_source/stage_a_source_manifest.jsonl
RUN_NAME=${RUN_NAME:-stage07_b1_checkpoint_gate32_v1}
CHECKPOINT_ITERS=${CHECKPOINT_ITERS:-"600 1000"}
...
MEGATRON_ROOT=$ROOT/checkpoints/uniss_streamspeech_ctc_v1/stage06_b1_megatron_v2
...
for iteration in $CHECKPOINT_ITERS; do
  ...
  for rank in 0 1 2 3 4 5 6 7; do
    ...
    CUDA_VISIBLE_DEVICES="$rank" "$PYTHON" "$STAGE/evaluate_text_probe.py" \
      ...
      --stage04-b2-checkpoint "$ROOT/checkpoints/uniss_streamspeech_ctc_v1/stage04_b2_phase3_endpoint_v1/best.pt" \
      --megatron-checkpoint "$checkpoint" \
  ...
  (( failed == 0 )) || {
    echo "Stage07 probe failed for $checkpoint_name" >&2
    exit 1
  }
  "$PYTHON" "$STAGE/merge_text_probe.py" \
...
"$PYTHON" "$STAGE/compare_checkpoint_gate.py" \
