#!/usr/bin/env python3
"""Compare Stage06 B1 candidates against B2 and the planned S3 quality gate."""
...
def candidate_row(payload: dict) -> dict:
    ...
    return {
        "name": f"B1 iter {summary['checkpoint_iteration']}",
...
def main() -> None:
    ...
    baseline = {
        ...
        "average_bleu": (
            float(baseline_summary["bleu"]["eng->cmn"]["score"])
            + float(baseline_summary["bleu"]["cmn->eng"]["score"])
        )
        / 2,
    }
    candidates = [
    ...
    for row in candidates:
        ...
        row["passes_s3_gate"] = (
            row["en_zh_bleu"] > args.en_zh_gate and row["zh_en_bleu"] > args.zh_en_gate
        )
    selected = max(candidates, key=lambda row: (row["average_bleu"], -row["iteration"]))
    output = {
        "schema_version": "uniss_streamspeech_stage07_checkpoint_gate_v1",
        "baseline": baseline,
        "candidates": candidates,
        "gate": {"en_zh_bleu": args.en_zh_gate, "zh_en_bleu": args.zh_en_gate},
        "selected": selected,
        "any_candidate_passes": any(row["passes_s3_gate"] for row in candidates),
    }
    ...
    for row in sorted(candidates, key=lambda value: value["iteration"]):
        lines.append(
            f"| B1 iter {row['iteration']} | {row['en_zh_bleu']:.4f} | "
            f"{row['zh_en_bleu']:.4f} | {row['delta_en_zh_vs_b2']:+.4f} | "
            f"{row['delta_zh_en_vs_b2']:+.4f} | "
            f"{'PASS' if row['passes_s3_gate'] else 'FAIL'} | "
            f"{row['compute_rtf_source_mean']:.4f} |"
        )
    lines.extend(
        [
            "",
            f"Planned S3 gate: EN→ZH > {args.en_zh_gate:.2f} and ZH→EN > {args.zh_en_gate:.2f}.",
            "",
            f"Selected fixed-probe candidate: iteration {selected['iteration']} by mean bidirectional BLEU.",
            "",
            "This gate uses the same 32 validation rows as the Stage04 probe. Passing it is required before "
            "generated-audio and streaming latency evaluation; it does not itself claim online latency.",
            "",
        ]
    )
