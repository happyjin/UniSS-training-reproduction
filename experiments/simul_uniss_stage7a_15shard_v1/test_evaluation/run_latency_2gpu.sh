...
if [[ $# -ne 5 ]]; then
  echo "Usage: $0 LABEL MODEL GPU_LIST PARENT_RUN_DIR BEST_STEP" >&2
  exit 2
fi
LABEL="$1"; MODEL="$2"; GPU_LIST="$3"; PARENT_RUN_DIR="$4"; BEST_STEP="$5"
...
STREAMING_MODE="stage7a_${LABEL}_latency_batch1"
export PYTHONPATH="${REPO_ROOT}:${PYTHONPATH:-}"
export VLLM_USE_V1=0 GLOO_SOCKET_IFNAME=lo TOKENIZERS_PARALLELISM=false
...
for rank in 0 1; do
  CUDA_VISIBLE_DEVICES="${GPU_IDS[${rank}]}" "${EVAL_ENV}/bin/python" \
    ...
    --model "${MODEL}" --schedules "${TEST_SCHEDULES}" --output-dir "${RUN_DIR}/generation" \
    --rank "${rank}" --world-size 2 --limit-records "${LATENCY_RECORDS}" \
    --batch-records 1 --max-write-tokens "${MAX_WRITE_TOKENS}" \
    --repetition-penalty "${REPETITION_PENALTY}" --max-model-len "${MAX_MODEL_LEN}" \
    --training-context-limit "${TRAINING_CONTEXT_LIMIT}" --max-num-seqs 1 \
    --max-num-batched-tokens "${MAX_MODEL_LEN}" --max-seq-len-to-capture 4096 \
    --gpu-memory-utilization 0.50 --streaming-mode "${STREAMING_MODE}" --dtype bfloat16 --resume \
...
"${EVAL_ENV}/bin/python" -m evaluation.simultaneous_streaming.stage4_aggregate merge \
  ...
  --output "${RUN_DIR}/generation_results.jsonl" --expected-records "${LATENCY_RECORDS}" --expected-ranks 2
...
"${EVAL_ENV}/bin/python" -m evaluation.simultaneous_streaming.stage4_aggregate merge \
  ...
  --output "${RUN_DIR}/results.jsonl" --expected-records "${LATENCY_RECORDS}" --expected-ranks 2
...
"${EVAL_ENV}/bin/python" -m evaluation.simultaneous_streaming.stage4_aggregate report \
  ...
  --stage-description "Stage7A batch-one latency audit" --streaming-mode "${STREAMING_MODE}" \
