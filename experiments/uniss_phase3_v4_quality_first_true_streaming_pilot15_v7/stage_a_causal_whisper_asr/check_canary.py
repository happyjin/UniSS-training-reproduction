#!/usr/bin/env python3
"""Authorize v7 formal only after a post-decay short-chunk hold-canary."""
...
def parse_log(text: str) -> tuple[int, dict[str, float], int, int, int, bool]:
...
def evaluate(text: str) -> dict[str, object]:
    ...
    checks = {
        "training_reached_iteration_191": max_iteration >= FINAL_ITERATION,
        "post_decay_hold_completed": max_iteration - OPTIMIZER_HORIZON >= 64,
        "final_checkpoint_saved": final_saved,
        "final_validation_reached_iteration_191": iteration >= FINAL_ITERATION,
    ...
    return {
        "schema_version": "uniss_stage_a_v7_post_decay_canary_gate_v1",
        "passed": all(checks.values()),
        "validation_iteration": iteration,
        "max_training_iteration": max_iteration,
        "optimizer_horizon": OPTIMIZER_HORIZON,
        "post_decay_hold_updates": max(0, max_iteration - OPTIMIZER_HORIZON),
        "metrics": metrics,
