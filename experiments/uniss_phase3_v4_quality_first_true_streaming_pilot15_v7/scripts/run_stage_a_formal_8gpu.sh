...
if (
    value.get("schema_version") != "uniss_stage_a_v7_post_decay_canary_gate_v1"
    or not value.get("passed")
):
    raise SystemExit("Stage A v7 post-decay canary has not authorized formal training")
PY

RUN_ID=${RUN_ID:-stage_a_v7_optimizer127_formal8_$(date -u +%Y%m%dT%H%M%SZ)}
export RUN_ID
...
export RUN_MASTER_PORT=${RUN_MASTER_PORT:-29733}
