...
RUN_ID=${1:-episode_grpo_formal_8gpu_v1}
PACK_ID=${2:-formal_64x4_train_16x4_valid_v1}
EPOCHS=${3:-3}
TRAIN=${REPO_ROOT}/data/processed/uniss_phasea_stateful_longepisode_rl_v1/${PACK_ID}/train_packs.jsonl
VALID=${REPO_ROOT}/data/processed/uniss_phasea_stateful_longepisode_rl_v1/${PACK_ID}/valid_packs.jsonl
...
[[ "${EPOCHS}" =~ ^[1-9][0-9]*$ ]] || { echo "epochs must be positive" >&2; exit 2; }
[[ ! -e "${SAVE}" && ! -e "${TB}" && ! -e "${LOG}" ]] || {
...
TRAIN_RECORDS=$("${PYTHON}" - "${TRAIN}.offsets.bin" <<'PY'
import os,sys
...
CMD=(
  ...
  --eval-iters 2
  --eval-interval "${STEPS_PER_EPOCH}"
  --eval-micro-batch-size 1
  --eval-global-batch-size 8
  --log-interval 1
  --tensorboard-dir "${TB}"
