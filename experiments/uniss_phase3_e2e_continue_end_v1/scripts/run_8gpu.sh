...
# --- the three terms this experiment adds (see training/objective_ext.py) ----
# Every number below is read off the zero-training bias sweep in
# reports/uniss_phase3_e2e_speak_decision_v1/family_logit_probe/
# BIAS_SWEEP_ANALYSIS.zh-CN.md.
#
# continue_after_fragment: WRITE_GENERATE vs WAIT_READ on rows that are NOT the
# first decision of their event.  The probe put that decision's median gap at
# -2.88 while the first decision of an event leads by +28.58, and a +3 inference
# bias on it alone moved WRITE_MT per event 0.168 -> 0.863, semantic coverage
# 0.666 -> 0.997 and natural_eos 0.50 -> 1.00.  On gold data the two classes are
# already 0.98:1, so a single shared denominator is correct here; the previous
...
# text length ratio 1.03 -> 2.25 with a 44.45 worst case.  Mirrors
...
# ratio from 44.45 to 4.07 under an identical +4 bias, a 10.9x reduction, so it
# is no longer unvalidated.
REPETITION_WEIGHT=${REPETITION_WEIGHT:-0.3}
REPETITION_WINDOW=${REPETITION_WINDOW:-8}
...
RUN_SUMMARY=${RUN_REPORT_ROOT}/CONTINUE_END_RUN.json
...
output_path.write_text(
    json.dumps(
        {
            "schema_version": "uniss_e2e_continue_end_input_audit_v1",
            "status": "passed",
...
env \
  ...
  UNISS_E2E_CONTINUE_AFTER_FRAGMENT_WEIGHT="${CONTINUE_WEIGHT}" \
  UNISS_E2E_CONTINUE_AFTER_FRAGMENT_LOGIT_MARGIN="${CONTINUE_LOGIT_MARGIN}" \
  UNISS_E2E_CONTENT_END_MARGIN_WEIGHT="${CONTENT_END_WEIGHT_NEW}" \
  UNISS_E2E_CONTENT_END_LOGIT_MARGIN="${CONTENT_END_LOGIT_MARGIN}" \
  UNISS_E2E_REPETITION_WEIGHT="${REPETITION_WEIGHT}" \
  UNISS_E2E_REPETITION_WINDOW="${REPETITION_WINDOW}" \
  "${HERE}/scripts/run_e2e_megatron_continue_end.sh"
...
"${PYTHON_BIN}" -m \
  ...
  --candidate "continue_end=${FINAL_CHECKPOINT}" \
...
pathlib.Path("${RUN_SUMMARY}").write_text(json.dumps({
  "schema_version": "uniss_e2e_continue_end_v1",
  "status": "complete",
  "formal_training_authorized": False,
  ...
  "objective": {
    ...
    "continue_after_fragment_logit_margin": ${CONTINUE_LOGIT_MARGIN},
    "content_end_margin": ${CONTENT_END_WEIGHT_NEW},
    "content_end_logit_margin": ${CONTENT_END_LOGIT_MARGIN},
    "repetition_penalty": ${REPETITION_WEIGHT},
