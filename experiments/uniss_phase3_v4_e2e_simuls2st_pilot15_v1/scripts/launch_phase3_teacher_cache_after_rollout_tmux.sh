...
command="cd ${REPO_ROOT} && DATA_RUN_ID=${DATA_RUN_ID} V1_FORMAL_RUN_ID=${V1_FORMAL_RUN_ID} FORMAL_RUN_ID=${FORMAL_RUN_ID} ROLLOUT_SESSION=${ROLLOUT_SESSION:-uniss_e2e_v1_rollout_formal} WAIT_INTERVAL_SECONDS=${WAIT_INTERVAL_SECONDS:-30} NUM_GPUS=${NUM_GPUS:-8} PHASE3_PROCESSES_PER_GPU=${PHASE3_PROCESSES_PER_GPU:-1} V1_PROCESSES_PER_GPU=${V1_PROCESSES_PER_GPU:-1} PHASE3_SMOKE_LIMIT=${PHASE3_SMOKE_LIMIT:-256} V1_SMOKE_LIMIT=${V1_SMOKE_LIMIT:-64} QUALITY_AUDIT_WORKERS=${QUALITY_AUDIT_WORKERS:-64} ${SCRIPT_DIR}/wait_for_v1_then_run_phase3_teacher_cache.sh ${redirect} ${WATCH_LOG} 2>&1"
