...
command="cd ${REPO_ROOT} && DATA_RUN_ID=${DATA_RUN_ID} V1_FORMAL_RUN_ID=${V1_FORMAL_RUN_ID} TEACHER_FORMAL_RUN_ID=${TEACHER_FORMAL_RUN_ID} TASK_POOL_RUN_ID=${TASK_POOL_RUN_ID} TEACHER_SESSION=${TEACHER_SESSION} WAIT_INTERVAL_SECONDS=${WAIT_INTERVAL_SECONDS:-30} TASK_POOL_WORKERS=${TASK_POOL_WORKERS:-64} ${SCRIPT_DIR}/wait_for_teacher_caches_then_build_task_pools.sh > ${WATCH_LOG} 2>&1"
