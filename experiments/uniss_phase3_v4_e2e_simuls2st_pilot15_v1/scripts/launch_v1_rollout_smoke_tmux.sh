...
command="cd ${REPO_ROOT} && DATA_RUN_ID=${DATA_RUN_ID} ROLLOUT_RUN_ID=${ROLLOUT_RUN_ID} ROLLOUT_SPLIT=${ROLLOUT_SPLIT:-valid} ROLLOUT_LIMIT=${ROLLOUT_LIMIT:-32} NUM_WORKERS=${NUM_WORKERS:-8} ${SCRIPT_DIR}/run_v1_rollout_8gpu.sh"
