...
if [[ "${DATA_RUN_ID}" == "gold_smoke_v1" ]]; then
  echo "Phase3 teacher cache requires the explicit immutable gold DATA_RUN_ID" >&2
  exit 2
fi
...
TEACHER_LIMIT=${TEACHER_LIMIT:-256}
NUM_GPUS=${NUM_GPUS:-8}
PROCESSES_PER_GPU=${PROCESSES_PER_GPU:-1}
NUM_WORKERS=$((NUM_GPUS * PROCESSES_PER_GPU))
TOPK=${TOPK:-32}
TEMPERATURE=${TEMPERATURE:-1.5}
SEMANTIC_STRIDE=${SEMANTIC_STRIDE:-8}
SAMPLE_GROUP_SIZE=${SAMPLE_GROUP_SIZE:-8}
RECORDS_PER_BUNDLE=${RECORDS_PER_BUNDLE:-64}
MAX_PADDED_TOKENS=${MAX_PADDED_TOKENS:-65536}
MAX_BATCH_SIZE=${MAX_BATCH_SIZE:-64}
MAX_SELECTED_POSITIONS=${MAX_SELECTED_POSITIONS:-512}
...
TEACHER_ROOT=${PROCESSED_ROOT}/phase3_teacher_cache/${TEACHER_RUN_ID}
TEACHER_REPORT_ROOT=${REPORT_ROOT}/phase3_teacher_cache/${TEACHER_RUN_ID}
TEACHER_LOG_ROOT=${LOG_ROOT}/phase3_teacher_cache/${TEACHER_RUN_ID}
PARTS_ROOT=${TEACHER_ROOT}/parts
MERGED=${TEACHER_ROOT}/${TEACHER_SPLIT}_phase3_teacher_cache.jsonl
AUDIT=${TEACHER_REPORT_ROOT}/AUDIT.json
GPU_SUMMARY=${TEACHER_REPORT_ROOT}/GPU_SUMMARY.json
HF_FINGERPRINT=${TEACHER_REPORT_ROOT}/PHASE3_HF_FINGERPRINT.json

for path in "${TEACHER_ROOT}" "${TEACHER_REPORT_ROOT}" "${TEACHER_LOG_ROOT}"; do
  if [[ -e "${path}" ]]; then
    echo "refusing to overwrite Phase3 teacher-cache run: ${path}" >&2
    exit 2
  fi
done
for path in "${GOLD}" "${ROLLOUT}" "${ROLLOUT_AUDIT}" "${ROLLOUT_QUALITY}" "${PHASE3_HF_MODEL}"; do
  if [[ ! -e "${path}" ]]; then
    echo "required Phase3 teacher-cache input is missing: ${path}" >&2
    exit 2
  fi
done
...
"${PYTHON_BIN}" -m experiments.uniss_phase3_v4_e2e_simuls2st_pilot15_v1.data.fingerprint \
  --checkpoint "phase3_hf=${PHASE3_HF_MODEL}" \
  --output "${HF_FINGERPRINT}" \
  --workers 16 \
  > "${TEACHER_LOG_ROOT}/hf_fingerprint.log" 2>&1
PHASE3_HF_SHA=$("${PYTHON_BIN}" -c 'import json,sys; print(json.load(open(sys.argv[1]))["checkpoints"]["phase3_hf"]["sha256"])' "${HF_FINGERPRINT}")
...
for ((worker=0; worker<NUM_WORKERS; worker++)); do
  ...
  CUDA_VISIBLE_DEVICES=${gpu} "${PYTHON_BIN}" -m \
    experiments.uniss_phase3_v4_e2e_simuls2st_pilot15_v1.teacher.build_phase3_cache \
    --gold "${GOLD}" \
    --rollouts "${ROLLOUT}" \
    --output-dir "${part}" \
    --model "${PHASE3_HF_MODEL}" \
    --phase3-hf-sha256 "${PHASE3_HF_SHA}" \
    --rank "${worker}" \
    --world-size "${NUM_WORKERS}" \
    --start-index "${TEACHER_START_INDEX}" \
    --topk "${TOPK}" \
    --temperature "${TEMPERATURE}" \
    --semantic-stride "${SEMANTIC_STRIDE}" \
    --sample-group-size "${SAMPLE_GROUP_SIZE}" \
    --records-per-bundle "${RECORDS_PER_BUNDLE}" \
    --max-padded-tokens "${MAX_PADDED_TOKENS}" \
    --max-batch-size "${MAX_BATCH_SIZE}" \
    --max-selected-positions "${MAX_SELECTED_POSITIONS}" \
    --device cuda:0 \
    "${limit_args[@]}" \
    > "${log}" 2>&1 &
  pids+=("$!")
...
if (( status != 0 )); then
  echo "Phase3 teacher-cache worker failed; inspect ${TEACHER_LOG_ROOT}" >&2
  exit "${status}"
fi
cleanup
MONITOR_PID=""
...
"${PYTHON_BIN}" -m \
  experiments.uniss_phase3_v4_e2e_simuls2st_pilot15_v1.teacher.merge_phase3_cache \
  --gold "${GOLD}" \
