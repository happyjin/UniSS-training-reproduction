...
# --- the two terms this experiment adds (see training/objective_ext.py) ------
# speak_decision: WRITE_GENERATE vs WAIT_READ, softplus-calibrated, each class
# normalised separately so the measured 0.863 / 0.168 imbalance cannot decide
# the gradient.  Scaled to match semantic_end_ce.
SPEAK_DECISION_WEIGHT=${SPEAK_DECISION_WEIGHT:-0.5}
SPEAK_DECISION_LOGIT_MARGIN=${SPEAK_DECISION_LOGIT_MARGIN:-1.0}
# repetition_penalty: probability mass on tokens already emitted in the same
# fragment.  Deliberately small; it is new and unvalidated, and a large value
# would distort the cross-entropy.
REPETITION_WEIGHT=${REPETITION_WEIGHT:-0.1}
...
OWN_NAME=uniss_phase3_e2e_speak_decision_v1
...
RUN_SUMMARY=${RUN_REPORT_ROOT}/SPEAK_DECISION_RUN.json
...
output_path.write_text(
    json.dumps(
        {
            "schema_version": "uniss_e2e_speak_decision_input_audit_v1",
            "status": "passed",
...
env \
  ...
  UNISS_E2E_SPEAK_DECISION_WEIGHT="${SPEAK_DECISION_WEIGHT}" \
  UNISS_E2E_SPEAK_DECISION_LOGIT_MARGIN="${SPEAK_DECISION_LOGIT_MARGIN}" \
  UNISS_E2E_REPETITION_WEIGHT="${REPETITION_WEIGHT}" \
  UNISS_E2E_REPETITION_WINDOW="${REPETITION_WINDOW}" \
  "${HERE}/scripts/run_e2e_megatron_speak.sh"
...
"${PYTHON_BIN}" -m \
  ...
  --candidate "speak=${FINAL_CHECKPOINT}" \
...
pathlib.Path("${RUN_SUMMARY}").write_text(json.dumps({
  "schema_version": "uniss_e2e_speak_decision_v1",
  "status": "complete",
  "formal_training_authorized": False,
  ...
  "objective": {
    ...
    "speak_decision": ${SPEAK_DECISION_WEIGHT},
    "speak_decision_logit_margin": ${SPEAK_DECISION_LOGIT_MARGIN},
