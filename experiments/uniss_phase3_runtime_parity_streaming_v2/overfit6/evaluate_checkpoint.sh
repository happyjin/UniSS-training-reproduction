...
TAG="${TAG:-untied_parallel_v1}"
CHECKPOINT="${SAVE_DIR}/iter_$(printf '%07d' "${ITERATION}")"
EVAL_ROOT="${REPO_ROOT}/reports/uniss_phase3_runtime_parity_streaming_v2/overfit6_v1_${TAG}/iter_$(printf '%07d' "${ITERATION}")"
EXPORT_ROOT="${REPO_ROOT}/reports/uniss_phase3_runtime_parity_streaming_v2/runtime_exports/overfit6_v1_iter_$(printf '%07d' "${ITERATION}")_${TAG}"
...
for value in "${CHECKPOINT}/.metadata" "${BASE_MODEL}/config.json" \
  ...
  "${INFERENCE_PYTHON}"; do
  [[ -e "${value}" ]] || { echo "Missing overfit6 evaluation input: ${value}" >&2; exit 1; }
done
...
exec "${INFERENCE_PYTHON}" -m web_demo.runtime_parity_streaming_v6.evaluate_checkpoint \
