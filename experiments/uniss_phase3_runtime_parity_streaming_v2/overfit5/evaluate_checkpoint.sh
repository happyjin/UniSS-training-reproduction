...
TAG="${TAG:-parallel_semantic_v1}"
CHECKPOINT="${SAVE_DIR}/iter_$(printf '%07d' "${ITERATION}")"
EVAL_ROOT="${REPO_ROOT}/reports/uniss_phase3_runtime_parity_streaming_v2/overfit5_v1_${TAG}/iter_$(printf '%07d' "${ITERATION}")"
EXPORT_ROOT="${REPO_ROOT}/reports/uniss_phase3_runtime_parity_streaming_v2/runtime_exports/overfit5_v1_iter_$(printf '%07d' "${ITERATION}")_${TAG}"
...
for value in \
  ...
  [[ -e "${value}" ]] || { echo "Missing overfit5 evaluation input: ${value}" >&2; exit 1; }
done
[[ ! -e "${EVAL_ROOT}" ]] || {
  echo "Refusing to overwrite overfit5 evaluation: ${EVAL_ROOT}" >&2
  exit 1
}
...
exec "${INFERENCE_PYTHON}" -m web_demo.runtime_parity_streaming_v5.evaluate_checkpoint \
