...
TAG="${TAG:-held_out2_strict_v14_v1}"
...
for value in "${CHECKPOINT}/.metadata" "${BASE_MODEL}/config.json" \
  ...
  [[ -e "${value}" ]] || { echo "Missing generalize14 evaluation input: ${value}" >&2; exit 1; }
done
...
exec "${INFERENCE_PYTHON}" -m experiments.uniss_phase3_runtime_parity_streaming_v2.generalize14_dagger_prefix.evaluate_checkpoint \
