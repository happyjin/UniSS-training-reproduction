...
TAG="${TAG:-held_out2_strict_v15_v1}"
...
for value in "${CHECKPOINT}/.metadata" "${BASE_MODEL}/config.json" \
  ...
  [[ -e "${value}" ]] || { echo "Missing generalize15 evaluation input: ${value}" >&2; exit 1; }
done
...
exec "${INFERENCE_PYTHON}" -m experiments.uniss_phase3_runtime_parity_streaming_v2.generalize15_action_eos_calibration.evaluate_checkpoint \
  ...
  --samples "${SAMPLES}" --maximum-drain-ticks 16 --minimum-text-similarity 0.98 \
