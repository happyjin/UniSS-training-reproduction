...
for value in \
  ...
  [[ -f "${value}" ]] || { echo "Missing generalize15 artifact: ${value}" >&2; exit 1; }
done
...
if value.get("status") != "complete" or not value.get("strict_global_shuffle"):
    raise SystemExit("generalize15 training manifest is incomplete")
...
if [[ -s "${TRACKER}" ]]; then
  ...
  if (( current == TRAIN_ITERS )); then
    echo "generalize15 already complete at ${TRAIN_ITERS}"
    exit 0
  fi
  LOAD="${SAVE_DIR}"
  FINETUNE=0
  ...
  if [[ -e "${SAVE_DIR}" && -n "$(find "${SAVE_DIR}" -mindepth 1 -maxdepth 1 -print -quit 2>/dev/null)" ]]; then
    echo "Refusing to overwrite non-empty generalize15 checkpoint directory: ${SAVE_DIR}" >&2
    exit 1
  fi
...
frozen_scope=phase3_base_embeddings_output_frontend_qwen_lora_text_semantic_microblock_support_safe_commit
runtime_prefix_schedule=clean_10pct_then_one_round_0p10
wait_false_positive_weight=2
deadline_survival_weight=1
continuation_head=start_glm_vs_eos
eos_class_weight=8
...
RUN_ENTRYPOINT="${V15_DIR}/pretrain_generalize15.py" \
...
[[ "${actual}" == "${TRAIN_ITERS}" ]] || {
  echo "generalize15 stopped at ${actual}, expected ${TRAIN_ITERS}" >&2
  exit 1
}
