...
source_iteration="$(printf 'iter_%07d' "${V14_BASE_ITERATION}")"
source_checkpoint="${V14_CHECKPOINT_ROOT}/${source_iteration}"
[[ -f "${source_checkpoint}/.metadata" ]] || {
  echo "Missing Generalize14 base checkpoint: ${source_checkpoint}" >&2
  exit 1
}
...
if [[ -L "${pinned_checkpoint}" ]]; then
  [[ "$(readlink -f "${pinned_checkpoint}")" == "$(readlink -f "${source_checkpoint}")" ]] || {
    echo "Pinned Generalize14 checkpoint points to a different source" >&2
    exit 1
  }
...
printf '%s\n' "${V14_BASE_ITERATION}" > "${PHASE3_NATIVE_CHECKPOINT}/latest_checkpointed_iteration.txt"
