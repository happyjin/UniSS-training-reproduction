"""Load Generalize15 heads beside the exact Phase3 HF runtime."""
...
from experiments.uniss_phase3_runtime_parity_streaming_v2.generalize15_action_eos_calibration.pretrain_generalize15 import (
    RuntimeParityGeneralize15Objective,
)
...
def load_runtime_models(export_dir: Path, *, codebook_weight, device, dtype=None):
    ...
    if missing_adapter or [name for name in unexpected if name in adapter]:
        raise ValueError("runtime v15 LoRA state does not match the exact HF graph")
    model.to(device=device, dtype=dtype).eval().requires_grad_(False)

    objective = RuntimeParityGeneralize15Objective(
    ...
    if missing != ["codebook.weight"] or unexpected:
        raise ValueError(
            f"runtime v15 objective mismatch: missing={missing}, unexpected={unexpected}"
        )
