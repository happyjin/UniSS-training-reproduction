...
EVAL_ROOT="${REPO_ROOT}/reports/uniss_phase3_runtime_parity_streaming_v2/generalize11_full15_v1_${TAG}/iter_$(printf '%07d' "${ITERATION}")"
EXPORT_ROOT="${REPO_ROOT}/reports/uniss_phase3_runtime_parity_streaming_v2/runtime_exports/generalize11_full15_v1_iter_$(printf '%07d' "${ITERATION}")_${TAG}"
...
for value in "${CHECKPOINT}/.metadata" "${BASE_MODEL}/config.json" \
  ...
  [[ -e "${value}" ]] || { echo "Missing generalize11 evaluation input: ${value}" >&2; exit 1; }
done
