...
source "${REPO_ROOT}/experiments/uniss_phase3_runtime_parity_streaming_v2/generalize11_full15/config.env"
...
for value in \
  ...
  [[ -f "${value}" ]] || { echo "Missing generalize11 artifact: ${value}" >&2; exit 1; }
done
...
if value.get("status") != "complete" or not value.get("strict_global_shuffle"):
    raise SystemExit("generalize11 training manifest is incomplete")
...
if [[ -s "${TRACKER}" ]]; then
  ...
  if (( current == TRAIN_ITERS )); then
    echo "generalize11 already complete at ${TRAIN_ITERS}"
    exit 0
  fi
  if (( current > TRAIN_ITERS )); then
  ...
  if [[ -e "${SAVE_DIR}" && -n "$(find "${SAVE_DIR}" -mindepth 1 -maxdepth 1 -print -quit 2>/dev/null)" ]]; then
    echo "Refusing to overwrite non-empty generalize11 checkpoint directory: ${SAVE_DIR}" >&2
    exit 1
  fi
...
RUN_ENTRYPOINT="${REPO_ROOT}/experiments/uniss_phase3_runtime_parity_streaming_v2/generalize11_full15/pretrain_generalize11.py" \
...
[[ "${actual}" == "${TRAIN_ITERS}" ]] || {
  echo "generalize11 stopped at ${actual}, expected ${TRAIN_ITERS}" >&2
  exit 1
}
