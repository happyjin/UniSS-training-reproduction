#!/usr/bin/env python3
"""Megatron overfit v7: v6 content plus explicit natural-length posterior."""
...
import experiments.uniss_phase3_runtime_parity_streaming_v2.overfit6.pretrain_overfit6 as v6
...
V7_TERM_NAMES = (*TERM_NAMES, "nar_semantic_content", "nar_semantic_length")
V7_DIAGNOSTIC_NAMES = (
    *DIAGNOSTIC_NAMES,
    "nar_semantic_accuracy",
    "nar_semantic_length_accuracy",
...
V7_WEIGHTS = OrderedDict(v2.OVERFIT_WEIGHTS)
V7_WEIGHTS["boundary_continuity"] = 0.5
V7_WEIGHTS["ar_semantic_microblock"] = 1.0
V7_WEIGHTS["nar_semantic_content"] = 4.0
V7_WEIGHTS["nar_semantic_length"] = 2.0


class RuntimeParityOverfit7Objective(v6.RuntimeParityOverfit6Objective):
    def __init__(self, hidden_size: int, codebook_weight: torch.Tensor, **kwargs) -> None:
        ...
        self.semantic_block_head = NaturalLengthParallelSemanticBlockHead(
            hidden_size, maximum_semantic_tokens=24
        )

    def replay(self, logits, labels, loss_mask) -> ObjectiveOutput:
        output = v2.RuntimeParityOverfit2Objective.replay(
            self, logits, labels, loss_mask
        )
        ...
        terms["nar_semantic_content"] = zero_term(anchor)
        terms["nar_semantic_length"] = zero_term(anchor)
        diagnostics = OrderedDict(output.diagnostics)
        zero = anchor.detach().new_zeros(())
        for name in V7_DIAGNOSTIC_NAMES[len(DIAGNOSTIC_NAMES) :]:
    ...
    def trajectory(
        ...
        *,
        ...
        output = v2.RuntimeParityOverfit2Objective.trajectory(
            self,
            hidden,
            logits,
            labels,
        ...
        terms = OrderedDict(output.terms)
        terms["nar_semantic_content"] = block.content_term
        terms["nar_semantic_length"] = block.length_term
        diagnostics = OrderedDict(output.diagnostics)
        diagnostics.update(
            (
                ("nar_semantic_accuracy", block.token_accuracy),
                ("nar_semantic_length_accuracy", block.length_accuracy),
...
def distributed_overfit7_objective(output: ObjectiveOutput, *, progress: float):
    del progress
    if tuple(output.terms) != V7_TERM_NAMES:
        raise ValueError("overfit7 objective term order changed")
    if tuple(output.diagnostics) != V7_DIAGNOSTIC_NAMES:
        raise ValueError("overfit7 diagnostic order changed")
    numerators = torch.stack(
        [output.terms[name].numerator for name in V7_TERM_NAMES]
    )
    denominators = torch.stack(
        [output.terms[name].denominator.to(numerators.dtype) for name in V7_TERM_NAMES]
    )
    ...
    total = (local_means * numerators.new_tensor(list(V7_WEIGHTS.values()))).sum()
    ...
    metrics = OrderedDict(
        (name, global_means[index]) for index, name in enumerate(V7_TERM_NAMES)
    )
    diagnostics = torch.stack(
        [output.diagnostics[name].detach().float() for name in V7_DIAGNOSTIC_NAMES]
    )
    if dist.is_available() and dist.is_initialized():
    ...
    metrics.update(
        ...
        for index, name in enumerate(V7_DIAGNOSTIC_NAMES)
    )
...
def dense_output_processor(**kwargs) -> torch.Tensor:
    ...
    if hidden.ndim != 3 or hidden.shape[1] != 1 or logits.shape[1] != 1:
        raise ValueError("overfit7 TP=PP=1 expects flattened [tokens,1,*]")
    hidden = hidden[:, 0]
    logits = logits[:, 0]
    ...
    if context["sample_kind"] == "replay":
        ...
        raise ValueError(f"unknown overfit7 sample kind: {context['sample_kind']}")
    total, metrics = distributed_overfit7_objective(
        output, progress=float(context["progress"])
    )
    if tuple(metrics) != V7_METRIC_NAMES:
        raise AssertionError("overfit7 metric order changed")
    values = (total.float(), *[metrics[name].float() for name in V7_METRIC_NAMES])
    if not all(torch.isfinite(value).all() for value in values):
        raise FloatingPointError("non-finite overfit7 loss component")
    return torch.stack(values)


def main() -> None:
    v2.trajectory_token_weights = trajectory_token_weights
    dense.base.TrueSubsecondObjective = RuntimeParityOverfit7Objective
    dense._distributed_dense_objective = distributed_overfit7_objective
    dense._dense_output_processor = dense_output_processor
    dense.METRIC_NAMES = V7_METRIC_NAMES
    dense.base.METRIC_NAMES = V7_METRIC_NAMES
    v6.freeze_v4_except_parallel_head()
    dense.main()
