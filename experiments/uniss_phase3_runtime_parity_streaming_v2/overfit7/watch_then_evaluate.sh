...
while true; do
  ...
  if (( latest >= COVERAGE_EPOCHS )) && ! pgrep -f 'overfit7/pretrain_overfit7.py' >/dev/null; then
    break
  fi
  sleep 10
done
ITERATION="${COVERAGE_EPOCHS}" TAG=posterior_length_v1 \
  bash "${REPO_ROOT}/experiments/uniss_phase3_runtime_parity_streaming_v2/overfit7/evaluate_checkpoint.sh"
summary="${REPO_ROOT}/reports/uniss_phase3_runtime_parity_streaming_v2/overfit7_v1_posterior_length_v1/iter_$(printf '%07d' "${COVERAGE_EPOCHS}")/summary.json"
"${PYTHON}" - "${summary}" <<'PY'
import json, sys
