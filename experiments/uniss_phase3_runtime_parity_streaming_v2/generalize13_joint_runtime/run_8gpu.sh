...
for value in \
  ...
  [[ -f "${value}" ]] || { echo "Missing generalize13 artifact: ${value}" >&2; exit 1; }
done
...
if value.get("status") != "complete" or not value.get("strict_global_shuffle"):
    raise SystemExit("generalize13 training manifest is incomplete")
...
if [[ -s "${TRACKER}" ]]; then
  ...
  if (( current == TRAIN_ITERS )); then
    echo "generalize13 already complete at ${TRAIN_ITERS}"
    exit 0
  fi
  LOAD="${SAVE_DIR}"
  FINETUNE=0
  ...
  if [[ -e "${SAVE_DIR}" && -n "$(find "${SAVE_DIR}" -mindepth 1 -maxdepth 1 -print -quit 2>/dev/null)" ]]; then
    echo "Refusing to overwrite non-empty generalize13 checkpoint directory: ${SAVE_DIR}" >&2
    exit 1
  fi
...
runtime_text_loss=frequency_balanced_delta_text_ce_weight_4
runtime_boundary_loss=critical_boundary_ce_weight_1
runtime_action_loss=natural_action_ce_weight_1
phase3_replay_weight=1
validation=trajectory_only_with_dp_synchronization_strict_pcm_gate_unpadded
...
RUN_ENTRYPOINT="${V13_DIR}/pretrain_generalize13.py" \
...
[[ "${actual}" == "${TRAIN_ITERS}" ]] || {
  echo "generalize13 stopped at ${actual}, expected ${TRAIN_ITERS}" >&2
  exit 1
}
