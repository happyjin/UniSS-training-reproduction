...
OWN_NAME=uniss_phase3_e2e_rollin_continue_v1
...
RUN_SUMMARY=${RUN_REPORT_ROOT}/ROLLIN_CONTINUATION.json
...
output_path.write_text(
    json.dumps(
        {
            "schema_version": "uniss_e2e_rollin_continuation_input_audit_v1",
            "status": "passed",
...
"${PYTHON_BIN}" -m \
  ...
  --candidate "rollin=${FINAL_CHECKPOINT}" \
...
pathlib.Path("${RUN_SUMMARY}").write_text(json.dumps({
  "schema_version": "uniss_e2e_rollin_continuation_v1",
  "status": "complete",
  "formal_training_authorized": False,
