...
cmd=(python -m training.simul_uniss.full_data_pipeline schedule
  ...
  --stage3-epochs "${FULL_STAGE3_EPOCHS}" --stage4-epochs "${FULL_STAGE4_EPOCHS}"
  --stage6-epochs "${FULL_STAGE6_EPOCHS}" --warmup-fraction "${FULL_WARMUP_FRACTION}")
if [[ "${DRY_RUN}" == "1" ]]; then printf '%q ' "${cmd[@]}"; printf '\n'; exit 0; fi
[[ -f "${FULL_DATA_READY_MARKER}" && -f "${DATA_ASSEMBLY_MARKER}" ]] || exit 1
[[ ! -e "${TRAINING_SCHEDULE_FILE}" ]] || { echo "Refusing to overwrite ${TRAINING_SCHEDULE_FILE}" >&2; exit 1; }
