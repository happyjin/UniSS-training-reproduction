...
from experiments.uniss_phasea_event_constrained_grpo_long_v2.training.trace_generator import (
    EventTraceGenerator,
)
...
def main() -> None:
    ...
    try:
        with trajectory_path.open("w", encoding="utf-8") as trajectory_file:
            for episode_index, row in enumerate(rows):
                ...
                for group_index in range(args.group_size):
                    ...
                    reward = score_episode(observation, baseline)
                    candidates.append(
                        {
                        ...
                        json.dumps(
                            {
                                ...
                                "coverage": observation.spoken_text_fraction,
                ...
                for candidate in candidates:
                    for trace_index, trace in enumerate(candidate.pop("traces")):
                        trajectory_file.write(
                            json.dumps(
                                {
                                    "schema_version": "uniss_event_constrained_trajectory_v2",
        ...
        controller.close()
    payload = {
        "schema_version": "uniss_event_constrained_rollout_v2",
        "status": "complete",
        "round_index": args.round_index,
        ...
        "route_semantics": "asr_frozen_adapter_on_control_mt_tts",
