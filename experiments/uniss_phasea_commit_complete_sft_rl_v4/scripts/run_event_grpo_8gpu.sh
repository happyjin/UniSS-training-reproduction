...
RUN_ID=${1:?usage: run_event_grpo_8gpu.sh RUN_ID PACK_ID LOAD_ROOT}
PACK_ID=${2:?missing pack ID}
LOAD_ROOT=${3:?missing previous checkpoint root}
DATA=${REPO_ROOT}/data/processed/uniss_phasea_commit_complete_sft_rl_v4/${PACK_ID}
TRAIN=${DATA}/train_packs.jsonl
VALID=${DATA}/valid_packs.jsonl
SAVE=${REPO_ROOT}/checkpoints/uniss_phasea_commit_complete_sft_rl_v4/${RUN_ID}
TB=${REPO_ROOT}/runs/uniss_phasea_commit_complete_sft_rl_v4/tensorboard/${RUN_ID}
LOG=${REPO_ROOT}/logs/uniss_phasea_commit_complete_sft_rl_v4/${RUN_ID}.log
for path in "${TRAIN}" "${TRAIN}.offsets.bin" "${VALID}" "${VALID}.offsets.bin"; do
  [[ -f "${path}" ]] || { echo "missing ${path}" >&2; exit 2; }
done
[[ -f "${LOAD_ROOT}/latest_checkpointed_iteration.txt" ]] || {
  echo "missing checkpoint root ${LOAD_ROOT}" >&2; exit 2;
}
[[ ! -e "${SAVE}" && ! -e "${TB}" && ! -e "${LOG}" ]] || {
...
export UNISS_E2E_COMPILE_CACHE_ROOT=/opt/dlami/nvme/jasonleeeli/.cache/uniss_commit_v4_grpo/${RUN_ID}
...
CMD=(
  "$(dirname "${PYTHON}")/torchrun" --nproc_per_node 8 --master_port 29992
  "${REPO_ROOT}/experiments/uniss_phasea_coverage_constrained_grpo_v3/training/pretrain_event_grpo.py"
  ...
  --event-top-layers 8 --event-adapter-lr 5e-6
  --event-clip-epsilon 0.20 --event-kl-beta 0.03
  --event-replay-weight 0.50 --event-anchor-weight 0.03
  "${SMOKE_ARGS[@]}"
  ...
  --lr 5e-6 --min-lr 5e-7 --lr-warmup-iters 1
