...
RUN_ID=${1:?usage: run_joint_commit_sft_8gpu.sh RUN_ID [DATA_ID]}
DATA_ID=${2:-commit_sft_min2_v1}
DATA=${REPO_ROOT}/data/processed/uniss_phasea_commit_complete_sft_rl_v4/${DATA_ID}/packs
...
EPOCHS=${EPOCHS:-3}
TRAIN_ITERS=${TRAIN_ITERS:-$((STEPS_PER_EPOCH * EPOCHS))}
...
CMD=(
  ...
  "${REPO_ROOT}/experiments/uniss_phasea_coverage_constrained_grpo_v3/training/pretrain_action_warmup.py"
  ...
  --event-action-weight 1.5 --event-payload-weight 1.0
  # Action+payload supervision must dominate this repair pass.  With 117
  # full replay packs, a 0.20 coefficient would still outweigh the sparse
  # event labels by more than 2x at token level.
  --event-replay-weight 0.08 --event-anchor-weight 0.03
  "${SMOKE_ARGS[@]}"
