#!/usr/bin/env python3
"""Pack one fresh v4 COMMIT-aware rollout round with Phase3 replay."""
...
def main() -> None:
    ...
    # An eight-episode smoke/evaluation set can legitimately hash to no
    # validation rows.  Preserve deterministic episode-level separation while
    # ensuring Megatron always receives a non-empty validation stream.
    if not valid_rows and rows:
        held_out = max(str(row["episode_id"]) for row in rows)
        valid_rows = [row for row in rows if str(row["episode_id"]) == held_out]
        train_rows = [row for row in rows if str(row["episode_id"]) != held_out]
    ...
    report = {
        "schema_version": "uniss_content_gated_commit_dataset_v4",
        "status": "passed",
