"""Stateful Phase-A cascade with content-gated COMMIT semantics.

Unlike the prior event route, ASR and incremental MT are updated at every
causal observation.  The policy controls only whether an already-stable,
non-empty target increment is committed to TTS.  Consequently an arbitrary
WRITE logit can no longer create an empty TTS transaction or block upstream
ASR/MT state progression.
"""
...
def evaluate_event_policy_session(
    ...
    *,
    ...
    for block_start in range(0, len(source), PHYSICAL_BLOCK_SAMPLES):
        ...
        # Acoustic recognition is never gated by a speech-output action.  In
        # v3, an over-predicted WRITE token was the only path to update ASR/MT,
        # coupling content availability to a noisy control logit.
        ...
        if asr_text:
            ...
            if accepted:
                # Build a stable append-only text buffer independently of the
                # action.  The action below only drains this buffer to TTS.
                mt_new = state.mt_committer.update(mt_candidate, final=true_final)
                target_staging_ids.extend(mt_new)
            else:
                rejected_early_end += 1

        # WRITE means COMMIT a real, stable target increment.  When there is
        # no increment, a sampled WRITE is recorded but executed as WAIT; this
        # removes the v3 empty-write/TTS-failure shortcut.  A deadline may
        # drain available text, but can never manufacture a blank emission.
        actionable_commit = bool(target_staging_ids)
        deadline_forced_write = bool(
            not true_final
            and sampled_action == "WAIT"
            and actionable_commit
            and source_end_ms - last_executed_write_ms >= 4_000
        )
        executed_action = (
            "WRITE"
            if true_final
            or (actionable_commit and (sampled_action == "WRITE" or deadline_forced_write))
            else "WAIT"
        )
        if executed_action == "WRITE" and actionable_commit:
    ...
    result = {
        ...
        "runtime_mode": "phase_a_stateful_cascade_content_gated_commit_v4",
