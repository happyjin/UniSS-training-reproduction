...
RUN_ID=${RUN_ID:-stage_a_v6_horizon42_hold_canary8_$(date -u +%Y%m%dT%H%M%SZ)}
export RUN_ID
export RUN_ENTRYPOINT="${V6_ENTRYPOINT}"
export RUN_COMPILE_NAMESPACE=stage_a_v6_compile
...
export RUN_SAVE_DIR="${CHECKPOINT_ROOT}/stage_a_hold_canary/${RUN_ID}"
export RUN_TENSORBOARD_DIR="${RUN_ROOT}/stage_a_hold_canary/${RUN_ID}/tensorboard"
export RUN_LOG="${LOG_ROOT}/stage_a_hold_canary/${RUN_ID}/train.log"
export RUN_SEQ_LENGTH=18000
export RUN_MBS=1
export RUN_GBS=128
export RUN_COVERAGE_EPOCHS=1
export RUN_TRAIN_ITERS=127
export RUN_CURRICULUM_ITERS=42
export RUN_MAX_ACOUSTICS=2
export RUN_NUM_WORKERS=4
export RUN_MASTER_PORT=${RUN_MASTER_PORT:-29730}
...
exec bash "${EXPERIMENT_DIR}/scripts/run_stage_a_megatron_v6.sh" "$@"
