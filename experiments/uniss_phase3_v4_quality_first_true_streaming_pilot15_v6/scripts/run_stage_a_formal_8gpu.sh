...
if (
    value.get("schema_version") != "uniss_stage_a_v6_hold_canary_gate_v1"
    or not value.get("passed")
):
    raise SystemExit("Stage A v6 hold-canary has not authorized formal training")
PY

RUN_ID=${RUN_ID:-stage_a_v6_horizon127_formal8_$(date -u +%Y%m%dT%H%M%SZ)}
export RUN_ID
...
export RUN_MASTER_PORT=${RUN_MASTER_PORT:-29731}
