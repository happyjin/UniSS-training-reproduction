#!/usr/bin/env python3
"""Megatron Stage A v6 with a curriculum horizon independent of train iters."""
...
def effective_curriculum_progress(
    ...
    """Return clamped progress using the explicit curriculum horizon."""

    if consumed_samples < 0 or global_batch_size <= 0 or curriculum_iters <= 0:
        raise ValueError("invalid v6 curriculum position")
...
def add_experiment_args(parser):
    ...
    group = parser.add_argument_group(title="UniSS quality-first Stage A v6")
...
def validate_experiment_args(args) -> None:
    ...
    if curriculum_iters <= 0 or curriculum_iters > train_iters:
        raise ValueError("v6 curriculum iters must be in [1, train_iters]")
    _CURRICULUM_SCALE = train_iters / curriculum_iters


def curriculum_group_multiplier(group, progress: float) -> float:
    """Apply v5 parameter gates on effective rather than total progress."""

    if not 0.0 <= progress <= 1.0:
        raise ValueError("invalid Stage A v6 LR progress")
