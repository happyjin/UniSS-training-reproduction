...
def value(row: dict[str, Any], name: str) -> float:
    ...
    if name in row.get("coverage_audit", {}):
        return float(row["coverage_audit"][name])
    if name in {
        "target_coverage",
        ...
        # Historical arms predate the coverage audit.  Keep them displayable
        # without silently inventing a metric for an old rollout.
        return float("nan")
    result = row["result"]
    mapping = {
        "rtf": "rtf",
...
def main() -> None:
    ...
    machine = {
        "schema_version": "uniss_coverage_constrained_grpo_v3_evaluation_v1",
        "status": "passed",
    ...
    for arm in arms:
        ...
        lines.append(
            ...
            f"{fmt(metric['target_coverage']['mean'])}/{fmt(metric['spoken_target_coverage']['mean'])} | "
