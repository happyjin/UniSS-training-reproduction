...
def local_event_rewards(
    ...
    for event in events:
        ...
        empty_write = float(coverage.get("empty_write", 0.0))
        language_leak = float(coverage.get("language_leak", 0.0))
        # Reward new teacher-aligned content, not the raw number of TTS calls.
        # The former cannot be gamed by fragmenting one sentence into many
        # small writes; the latter was the dominant failure mode of v2.
        reward += 4.0 * target_delta + 2.0 * spoken_delta
        reward -= 0.50 * empty_write + 0.50 * language_leak
        ...
        if healthy:
            if last_audio_ms is None:
                reward -= 0.15 * math.log1p(source_ms / 1_000.0)
            else:
                reward -= 0.25 * softplus((source_ms - last_audio_ms - 2_000.0) / 1_000.0)
        ...
        if bool(event.get("true_source_final", False)):
            terminal_coverage = float(coverage.get("target_coverage", 0.0))
            reward += 1.00 if terminal_coverage >= 0.80 and bool(event.get("flush_complete", False)) else -2.00
        ...
        for trace in candidate["traces"]:
            ...
            if family == "control":
                ...
                raw = (
                    ...
                    + 2.0 * float(terminal["target_coverage"])
                    + float(terminal["spoken_target_coverage"])
                    - 3.0 * float(terminal["mt_shortfall"])
                    - 3.0 * float(terminal["completeness_shortfall"])
