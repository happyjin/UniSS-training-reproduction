...
> **2026-08-31 更正**:本节原第 1 条("把 local agreement 用到交织 S2S 会话上")**不成立,已删除**。
> 复查 `PersistentInterleavedSession.run_event` 后确认:交织会话是单一持久自回归流,模型被喂入已提交历史后只产出新 delta,**从不重新生成同一段内容**。local agreement 需要同一内容的两次假设才能取公共前缀,这里没有第二次假设,因此该策略无信号可用 —— `target_rollback_events = 0` 是平凡成立的,不是策略的功劳。真正的第 1 优先级是下面的语音长度控制。

1. **语音长度控制 —— 这是最大阻塞,且可精确定量。** 语义 token 速率恒定 **50 tok/s(20 ms/token)**,而事件推进源时间约 160–300 ms,所以 1:1 时长约束下每事件预算应为 **8–15 个 token**;**当前上限是 384,宽松了 25–48 倍**。逐条验证:`emilia_zh_0006199435` 源 6.06 s、1:1 预算 303 token、实际 685 → 2.26 倍;`emilia_zh_0004122419` 源 4.12 s、预算 206、实际 227 → 1.10 倍(因此它听起来正常)。
   同传系统的物理不变量是"输出音频不能比输入快",所以约束应当是**累计**而非逐事件的:
   $$\text{累计语义 token}\times 20\text{ms}\;\le\;\text{已消耗源时间}+\text{margin}$$
   这允许追赶式突发,同时由构造保证 `audio_over_source_ratio ≤ 1 + margin/T`。零训练、纯推理侧。
   **代价必须说清**:截断会降低内容覆盖率,并可能在短语中间切断。它把损害限定住、让 demo 可听且实时可行,**不是根治**。
2. **根治要靠训练侧的 END 决策。** 三个 checkpoint(iter 1132 / 1207 / 2264)的 `natural_eos` 全部是 0.50,说明现有 `semantic_end_ce=0.50` / `semantic_end_margin=0.25` 配置对收尾**完全无效**。需要把 END 监督与长度比直接关联,并在自由运行(roll-in)条件下监督 END,而当前 `semantic_rollin_*` 系列权重全为 0。注意这是离线继承缺陷(离线 `missing_eos` 92/256、时长比 3.06)。
