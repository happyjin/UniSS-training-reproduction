...
RealSI 777 条配对,相对训练前(部署音频,项目口径 asr_bleu_repo,
ASR-BLEU 的差值都配了 1000 次配对自举区间):

              静音(部署)   首次发声   片段    en->zh BLEU              zh->en BLEU
  2_paper    -0.0246 *     -97 ms    +0.73   -1.56 [-2.63,-0.42] *    -1.05 [-2.37,+0.54]
  3_guard    -0.0119 *     -63 ms    +0.62   -0.01 [-1.08,+1.11]      +0.28 [-0.97,+1.70]
  (* = 95% 区间不含零)

所以:
  3_guard 是纯收益 —— 停顿变少、开口更早,而翻译质量两个方向都持平。
  2_paper 停顿最少,但 en->zh 显著退步 1.56 —— 它冲过参考句长 17%,
          多说的部分有相当比例是错的(1-gram 精度 47.4 -> 42.7)。

要听的是:2_paper 少掉的那些停顿,值不值 1.56 BLEU。
