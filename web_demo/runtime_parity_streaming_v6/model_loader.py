"""Load the v6 objective with the exact Phase3 Hugging Face runtime."""
...
from experiments.uniss_phase3_runtime_parity_streaming_v2.overfit6.pretrain_overfit6 import (
    RuntimeParityOverfit6Objective,
)
...
def load_runtime_models(export_dir: Path, *, codebook_weight, device, dtype=None):
    ...
    if missing_adapter or [name for name in unexpected if name in adapter]:
        raise ValueError("runtime v6 LoRA state does not match the exact HF graph")
    model.to(device=device, dtype=dtype).eval().requires_grad_(False)
    objective = RuntimeParityOverfit6Objective(
    ...
    if missing != ["codebook.weight"] or unexpected:
        raise ValueError(
            f"runtime v6 objective mismatch: missing={missing}, unexpected={unexpected}"
        )
