...
GPU_ID="${UNISS_TRUE_STREAMING_GPU:-0}"
...
if [[ "${used}" -ge "${UNISS_TRUE_STREAMING_MAX_IDLE_MEMORY_MIB:-4096}" ]]; then
...
tmux new-session -d -s "${SESSION}" \
  "cd '${REPO_ROOT}' && CUDA_VISIBLE_DEVICES='${GPU_ID}' UNISS_TRUE_STREAMING_DEVICE='cuda:0' UNISS_TRUE_STREAMING_PORT='${PORT}' '${SCRIPT_DIR}/share_watchdog.sh'"
