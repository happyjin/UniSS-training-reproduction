"""Load the v7 objective with the exact Phase3 Hugging Face runtime."""
...
from experiments.uniss_phase3_runtime_parity_streaming_v2.overfit7.pretrain_overfit7 import (
    RuntimeParityOverfit7Objective,
)
...
def load_runtime_models(export_dir: Path, *, codebook_weight, device, dtype=None):
    ...
    if missing_adapter or [name for name in unexpected if name in adapter]:
        raise ValueError("runtime v7 LoRA state does not match the exact HF graph")
    model.to(device=device, dtype=dtype).eval().requires_grad_(False)
    objective = RuntimeParityOverfit7Objective(
    ...
    state = load_file(export_dir / "objective_model.safetensors")
    missing, unexpected = objective.load_state_dict(state, strict=False)
    if missing != ["codebook.weight"] or unexpected:
        raise ValueError(
            f"runtime v7 objective mismatch: missing={missing}, unexpected={unexpected}"
        )
