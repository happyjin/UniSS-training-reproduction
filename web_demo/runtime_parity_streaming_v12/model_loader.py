"""Load the v12 microblock objective beside the exact Phase3 HF runtime."""
...
def load_runtime_models(export_dir: Path, *, codebook_weight, device, dtype=None):
    ...
    if missing_adapter or [name for name in unexpected if name in adapter]:
        raise ValueError("runtime v12 LoRA state does not match the exact HF graph")
    model.to(device=device, dtype=dtype).eval().requires_grad_(False)

    objective = RuntimeParityGeneralize12Objective(
    ...
    if missing != ["codebook.weight"] or unexpected:
        raise ValueError(
            f"runtime v12 objective mismatch: missing={missing}, unexpected={unexpected}"
        )
