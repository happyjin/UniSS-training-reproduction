...
CONFIG_FILE="${CONFIG_FILE:-${REPO_ROOT}/configs/experiments/uniss_qwen0p5b_unist198_phase2_recovery_v2.env}"
...
if [[ "${DRY_RUN}" == "1" ]]; then
  ...
  printf '[dry-run] validate global-shuffle pilot through step %s with absolute grad norm <= %s\n' \
...
echo "Recovery v2 pilot passed; starting isolated full global-shuffle continuation."
"${runner[@]}" --mode full
