...
PHASE2_CHECKPOINT="${PHASE2_CHECKPOINT:-${REPO_ROOT}/checkpoints/uniss_qwen0p5b_phase2_unist13_full}"
HF_REFERENCE="${HF_REFERENCE:-${REPO_ROOT}/checkpoints/qwen2_0p5b_uniss_vocab_hf}"
HF_OUTPUT="${HF_OUTPUT:-${REPO_ROOT}/checkpoints/exported_hf/qwen0p5b_phase2_unist13_full_hf}"
...
OUTPUT_DIR="${OUTPUT_DIR:-${REPO_ROOT}/eval_outputs/qwen0p5b_phase2_unist13_s2st_${SPLIT}_${RUN_ID}}"
...
if [[ "${DRY_RUN}" == "0" ]]; then
  require_file "${PHASE2_CHECKPOINT}/latest_checkpointed_iteration.txt"
...
run_cmd "${REPO_ROOT}/scripts/convert_uniss_checkpoint.sh" export \
  ...
  --megatron-path "${PHASE2_CHECKPOINT}" \
  --hf-output "${HF_OUTPUT}" \
  --model-type gpt \
  --no-progress
...
run_cmd env \
  ...
  SPLIT="${SPLIT}" \
  STEP_NAME="qwen0p5b_phase2_unist13_s2st_audio" \
