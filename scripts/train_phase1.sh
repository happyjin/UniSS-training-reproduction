...
TRAIN_DATA="${TRAIN_DATA:-${REPO_ROOT}/data/megatron/phase1/packed_train.jsonl}"
VALID_DATA="${VALID_DATA:-}"
LOAD_CHECKPOINT="${LOAD_CHECKPOINT:-${REPO_ROOT}/checkpoints/qwen2_1p5b_uniss_vocab}"
SAVE_DIR="${SAVE_DIR:-${REPO_ROOT}/checkpoints/uniss_phase1}"
...
cmd=(torchrun
  ...
  --train-iters "${TRAIN_ITERS:-41667}"
  --lr 8e-4
  --min-lr 8e-4
  --lr-warmup-iters "${LR_WARMUP_ITERS:-13889}"
...
if [[ -n "${VALID_DATA}" ]]; then
  cmd+=(--uniss-packed-valid "${VALID_DATA}" --eval-iters "${EVAL_ITERS:-10}" --eval-interval "${EVAL_INTERVAL:-1000}")
else
  cmd+=(--eval-iters 0)
fi
